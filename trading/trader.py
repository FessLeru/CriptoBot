...
class Trader:
    ...
    def __init__(self, exchange: BitgetExchange):
        ...
        logger.info(f"Инициализирован трейдер с плечом {self.leverage} и размером позиции {self.position_size_percent}%")
    ...
    async def open_trade(self, signal: Dict) -> str:
        ...
        async with self._lock:
            ...
            try:
                ...
                if trail_mode:
                    if side == "buy":  # для лонга
                        trail_activation = price + trail_points  # активационная цена
                    else:  # для шорта
                        trail_activation = price - trail_points  # активационная цена
                        
                    trail_callback = (trail_offset / price) * 100  # процент отступа для API
                
                print(f"trail_activation: {trail_activation}, trail_callback: {trail_callback}")
                ...
                return f"✅ Открыта сделка {side.upper()} {symbol} на {order_amount:.4f} по {price:.4f} USDT{trail_msg}"
